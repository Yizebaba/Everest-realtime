...
截图失败或遇到拦截页时，不用数据卡冒充原文。通知正文保留来源链接、图片和最后一行署名：
`vx:No1-Shine ｜ 珠峰自然环境信息监控系统［测试版］`。
当前 `translate_screenshots: true`：使用 Google 官方网站翻译功能，将原站页面翻成简体中文后再截图，
保留网页布局，不重绘数据卡；原文截图单独保存在本地。中文图标注为机器翻译，翻译失败不会冒充成功。
页面内日期/时间是来源内容，采集开始、各页截图、来源获取完成和接口接收时间分别留在运行记录中。
